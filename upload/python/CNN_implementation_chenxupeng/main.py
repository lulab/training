...
def train_epoch(model, sess, X, y):
    ...
    while st < len(X) and ed <= len(X):
        ...
        times += 1
        sess.run(model.update_ema,feed)
    loss /= times
    acc /= times
    return acc, loss
...
with tf.Session() as sess:
    ...
    if FLAGS.is_train:
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            s[1] ="Epoch " + str(epoch + 1) + " of " + str(FLAGS.num_epochs) + " took " + str(epoch_time) + "s"
            s[2] ="  learning rate:                 " + str(cnn_model.learning_rate.eval())
            s[3] ="  training loss:                 " + str(train_loss)
            s[4] ="  validation loss:               " + str(val_loss)
            s[5] ="  validation accuracy:           " + str(val_acc)
            s[6] ="  best epoch:                    " + str(best_epoch)
            s[7] ="  best validation accuracy:      " + str(best_val_acc)
            s[8] ="  test loss:                     " + str(test_loss)
            s[9] ="  test accuracy:                 " + str(test_acc)
            for i in range(9):
                print s[i+1]
            ...
            logg = np.array([0]).astype('S')
            for i in range(9):
                logg = np.vstack((logg,np.array(s[i+1])))
            log[epoch] = logg
        log =np.array([val for (key,val) in log.iteritems()])
        np.savetxt('train.log',log,fmt = '%s')
